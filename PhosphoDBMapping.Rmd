...
```{r, eval = F}
vec <- paste0(tabK$SUB_ACC_ID, "_", tabK$SUB_MOD_RSD)
vec[vec == "NA_NA"] <- paste0(tabK$ACC_ID[vec == "NA_NA"], "_", gsub("-p", "", tabK$MOD_RSD[vec == "NA_NA"], fixed = T))
...
vec <- sapply(psiteInfo, function(x) {x$GeneID})
vec <- unlist(vec)
vec <- as.character(vec[names(psiteInfo) %in% notmapped])
