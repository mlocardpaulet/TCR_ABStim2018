...
For each phosphorylation site, when it is not measured in any biological replicate for a condition, I replace missing values with a random value generated around the 3% quantile of the intensities for each condition, using the standard deviation of the dataset (biological variance). 
...
```{r, eval = T}
mapping <- data.frame("ClusteringOuptut" = sort(unique(clusres$cluster)), "NewOrdering" = c(12,3,4,2,1,5,7,6,8,9,11,10))
