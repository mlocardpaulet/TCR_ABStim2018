...
**I cannot cluster sites with missing data so I remove them from the analysis.**

```{r}
clusmat <- na.omit(clusmat)
```
...
```{r, eval = T}
mapping <- data.frame("ClusteringOuptut" = sort(unique(clusres$cluster)), "NewOrdering" = c(12,2,5,6,1,3,11,4,9,8,7,10))
...
gtab1$facet <- ifelse(gtab1$Var1 %in% c(1, 2, 3, 4, 5, 6), "up", "down")
...
gtab2$facet <- ifelse(gtab2$Var1 %in% c(1, 2, 3, 4, 5, 6), "up", "down")
