...
*Actually there seems to be no real need to go through 200 loops for this analysis.*
...
```{r, eval = T}
mapping <- data.frame("ClusteringOuptut" = sort(unique(clusres$cluster)), "NewOrdering" = c(13,1,4,5,3,2,6,12,9,11,8,7,10))
...
gtab1$facet <- ifelse(gtab1$Var1 %in% c(1, 2, 3, 4, 5), "up", "down")
gtab1$facet[gtab1$Var1 == 13] <- "Mixed"
g1 <- ggplot(gtab1, aes(x = 0, y = Freq, fill = Var1)) +  geom_bar(width = 1, stat = "identity", position = "fill") + coord_polar("y") + ggtitle("Cluster composition of the regulated pY") + scale_fill_manual(values = colfuncClusters(13)[as.numeric(as.character(gtab1$Var1))]) + theme_minimal() + geom_text_repel(aes(x = 0.5, y = pos, label = Freq), nudge_x = .3, segment.size = .7, show.legend = FALSE)
ggplot(gtab1, aes(x = Var1, y = Freq, fill = Var1)) +  geom_bar(width = 1, stat = "identity", position = "dodge") + ggtitle("Cluster composition of the regulated pY") + scale_fill_manual(values = colfuncClusters(13)[as.numeric(as.character(gtab1$Var1))]) + theme_minimal() + facet_wrap(.~facet, scales = "free_x")
kable(gtab1)
...
gtab1$facet <- ifelse(gtab1$Var1 %in% c(1, 2, 3, 4, 5), "up", "down")
gtab1$facet[gtab1$Var1 == 13] <- "Mixed"
g2 <- ggplot(gtab1, aes(x = 0, y = Freq, fill = Var1)) +  geom_bar(width = 1, stat = "identity", position = "fill") + coord_polar("y") + ggtitle("Cluster composition of the regulated pS") + scale_fill_manual(values = colfuncClusters(13)[as.numeric(as.character(gtab1$Var1))]) + theme_minimal() + geom_text_repel(aes(x = 0.5, y = pos, label = Freq), nudge_x = .3, segment.size = .7, show.legend = FALSE)
ggplot(gtab1, aes(x = Var1, y = Freq, fill = Var1)) +  geom_bar(width = 1, stat = "identity", position = "dodge") + ggtitle("Cluster composition of the regulated pS") + scale_fill_manual(values = colfuncClusters(13)[as.numeric(as.character(gtab1$Var1))]) + theme_minimal() + facet_wrap(.~facet, scales = "free_x")
kable(gtab1) 
...
gtab1$facet <- ifelse(gtab1$Var1 %in% c(1, 2, 3, 4, 5), "up", "down")
gtab1$facet[gtab1$Var1 == 13] <- "Mixed"
g3 <- ggplot(gtab1, aes(x = 0, y = Freq, fill = Var1)) +  geom_bar(width = 1, stat = "identity", position = "fill") + coord_polar("y") + ggtitle("Cluster composition of the regulated pT") + scale_fill_manual(values = colfuncClusters(13)[as.numeric(as.character(gtab1$Var1))]) + theme_minimal() + geom_text_repel(aes(x = 0.5, y = pos, label = Freq), nudge_x = .3, segment.size = .7, show.legend = FALSE)
ggplot(gtab1, aes(x = Var1, y = Freq, fill = Var1)) +  geom_bar(width = 1, stat = "identity", position = "dodge") + ggtitle("Cluster composition of the regulated pT") + scale_fill_manual(values = colfuncClusters(13)[as.numeric(as.character(gtab1$Var1))]) + theme_minimal() + facet_wrap(.~facet, scales = "free_x")
kable(gtab1)
gtab2 <- rbind(gtab2, cbind(gtab1, "AminoAcid" = rep("pT", nrow(gtab1))))

gtab2$group <- paste0(gtab2$Var1, gtab2$`AminoAcid`)
gtab2$facet <- ifelse(gtab2$Var1 %in% c(1, 2, 3, 4, 5), "up", "down")
gtab2$facet[gtab2$Var1 == 13] <- "Mixed"
...
for (i in 1:length(ref)) {
  ...
  heatmap.2(hm[order(ord, decreasing = T),], mar = c(7, 10), main = paste0("log2(Mean quan values) of phosphosites in the cluster", el), col=colfunclight(100), cexCol = 0.9, trace = "none", Colv = F, cexRow = 0.35, colsep = c(4, 8, 12, 16, 20), keysize = 1.2, na.color = "grey", na.rm = T, Rowv = F, tracecol = "black", scale = "row")
