...
PlotSite <- function(psites, tab) { # psites is the vector of phosphosite ID to plot, tab is the table with the appropriate quantification values
  ...
  for (el in psites) {
    ...
    if (length(gtab$value[!is.na(gtab$value)]) > 0) {
      ...
      g <- ggplot(gtab, aes(x = TimePoint, y = value)) + geom_line(aes(x = TimePoint, y = value, group = Replicate, col = Replicate), size = 1.2)+ geom_vline(xintercept = 0, size = 1.2) + theme_minimal() + ggtitle(el) + scale_color_manual(values = colours[seq_along(unique(gtab$Replicate))]) + ylab("log2-transformed normalised MS intensities") # + geom_boxplot(aes(x = TimePoint, y = value), width = 0.5)
    } else {
